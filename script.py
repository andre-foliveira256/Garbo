...
def send_gotify_notification(material, ip_address):
    ...
    IP_ADDRESS = ip_address  # IP address of the Gotify server
    GOTIFY_URL = f"http://{IP_ADDRESS}/message?token={GOTIFY_TOKEN}"
        ...
        while True:
            ...
            for material, count in actual_garbage.items():
                if count >= GARBAGE_CAPACITY[material]:
                    ...
                    send_gotify_notification(material, ip_address)
...
if __name__ == "__main__":
    ...
    ip_address = input("Enter the IP address of the Gotify server: ")
    # Validate the IP address format (basic validation)
    try:
        socket.inet_aton(ip_address)  # This will raise an error if the IP address is invalid
        forward_data(ip_address)  # Call the function with the provided IP address
    except socket.error:
